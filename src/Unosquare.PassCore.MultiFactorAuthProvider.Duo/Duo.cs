...
#pragma warning disable CS1591, CS8603, CS1573, CS8625, CS8600, CS8602, CS8618
...
namespace Duo
{
    ...
    public class DuoApi
    {
        ...
        private JObject BaseJSONApiCall(string method,
                                string path,
                                Dictionary<string, string> parameters,
                                int timeout,
                                DateTime date)
        {
            ...
            try
            {
                var dict = JObject.Parse(res);
                if ((string)dict["stat"] == "OK")
                {
                    return dict;
                }
                else
                {
                    var code = dict.ContainsKey("code")
                        ? (int)dict["code"] : 0;
                    String message_detail = "";
                    if (dict.ContainsKey("message_detail"))
                    {
                        message_detail = (string)dict["message_detail"];
                    }
        ...
        public JObject JSONApiCall(string method,
                                string path,
                                Dictionary<string, string> parameters)
        {
        ...
        public T JSONApiCall<T>(string method,
            ...
            where T : class
        {
            return JSONApiCall(method, path, parameters, timeout, date).ToObject<T>();
        }
        ...
        public JObject JSONApiCall(string method,
                                ...
                                DateTime date)
        {
            return (JObject)BaseJSONApiCall(method, path, parameters, timeout, date)["response"];
        }

        public JObject JSONPagingApiCall(string method,
                               string path,
                               Dictionary<string, string> parameters,
                               int offset,
                               int limit)
        ...
        {
            return requestObject.Proxy != null;
        }
